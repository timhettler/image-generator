...
var BragBag = function (svgUrl, id) {
  var xcanvas,
      scale = 1,
      $canvas = $('<canvas/>'),
      canvasDimensions = {},
  ...
  this.init = function () {
    ...
    _self.fetchSvg(_self.svgUrl).then(function (svg) {
      xcanvas.compile (svg, function () {
        ...
        canvasDimensions.x = xcanvas.canvas.width;
        canvasDimensions.y = xcanvas.canvas.height;
  ...
  this.getPointData = function (layerToken) {
    var increment = textData[layerToken].size * 0.9,
        y = 0,
  ...
  this.wrapText = function (layerData, lineHeight) {
    ...
    for(var p = 0; p < layerData.length; p++) {
      var pData = layerData[p];
      var line = '';

      for(n; n < words.length; n++) {
        var testLine = line + words[n] + ' ';
        var metrics = ctx.measureText(testLine.toUpperCase());
        var testWidth = metrics.width;

        if (testWidth > pData.w) {
          ctx.fillText(testLine.toUpperCase(), pData.x, pData.y);
          n++;
          break;
        }
        else {
